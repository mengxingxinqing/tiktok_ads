...
**Path**: API Reference/Campaign/Create a campaign
...
**Online Data Terms**: By using this endpoint, to the extent that you and TikTok share personal data for the purposes of your campaign (for example, if you choose to use an advertiser tool such as TikTok Pixel or to work with a measurement partner), you agree to TikTok's Online Data Terms, including the [Business Products (Data) Terms](https://ads.tiktok.com/i18n/official/policy/business-products-terms), [Controller to Controller Data Terms](https://ads.tiktok.com/i18n/official/policy/controller-to-controller) and all component parts.

You agree that Section 13 of the [TikTok Commercial Terms of Service](https://ads.tiktok.com/i18n/official/policy/commercial-terms-of-service) shall apply to any and all liability arising under or in connection with the Online Data Terms, whether arising in contract, tort (including without limitation negligence) or otherwise.

For more information, see [TikTok Advertiser Tools and Related Terms](https://ads.tiktok.com/help/article/tiktok-advertiser-tools-and-related-terms?redirected=1&lang=en) and [TikTok Controller to Controller Data Terms](https://ads.tiktok.com/i18n/official/article?aid=619083947221699642). 

## Before you start
Review the step-by-step guides in [Campaign creation](https://business-api.tiktok.com/portal/docs?id=1758526151905281) to understand the parameter requirements for different types of campaigns or ads. If the type of ad or campaign you want to create is not covered in the existing guides and you would like to request additional documentation, contact your TikTok representative to submit the request.
...
|---|---|---|
|Endpoint path|/v1.2/campaign/create/|/v1.3/campaign/create/|
|Request parameter name| `budget_optimize_switch`
`industry_types`|`budget_optimize_on`
`special_industries`|
|Request parameter data type |`advertiser_id`:number|`advertiser_id`: string|
|New request parameters|/|`app_id`
 `campaign_type`
`app_promotion_type` 
...
- `APP_PREREGISTRATION`: App pre-registration. Get new users to pre-register before your app launches. To learn about how to create App Pre-registration ads, refer to [here](https://ads.tiktok.com/marketing_api/docs?id=1772549012687873).
...
- `WEB_AND_APP`: Website and app. Drive sales on both your website and your app.To learn about how to create ads with this setting, see [Create ads with Website and App Optimization](https://business-api.tiktok.com/portal/docs?id=1839876593708034).
...
|
| is_search_campaign| boolean | Whether to create a Search Ads Campaign.
With Search Ads Campaigns, you can select keywords and serve ads within TikTok's search result page. To learn more about how to create such campaigns, see [Create Search Ads Campaigns](https://business-api.tiktok.com/portal/docs?id=1815394136739841).
...
To learn more about Dedicated Campaigns, see [Dedicated Campaign](https://business-api.tiktok.com/portal/docs?id=1740029173531649).|
|app_id  {+Conditional}|string|Required when the campaign is an iOS 14 Dedicated Campaign (`camapign_type`=`IOS14_CAMPAIGN`) with the advertising objective (`objective_type`) as `PRODUCT_SALES`, or `APP_PROMOTION`( its `app_promotion_type` = `APP_INSTALL`).
...
- If you are allowlisted for disabling SKAN attribution for Dedicated Campaigns, you cannot set `bid_type` to `BID_TYPE_CUSTOM` at the ad group level when the following conditions are all met:At the campaign level: `is_advanced_dedicated_campaign` is `false`.
- `disable_skan_campaign` is `false`.
- At the ad group level:`optimization_goal` is `INSTALL` or `IN_APP_EVENT`.
- Once set, this field cannot be updated.
...
|rf_campaign_type |string| When `objective_type` is specified as `RF_REACH`, use this field to set the campaign as a TikTok Pulse campaign, then you can get available premium contextual tags. 
...
- If you select `rf_campaign_type` to `PULSE` , then you must enter contextual tags of `PREMIUM` type in the `contextual_tag_ids`  field in [/adgroup/rf/create/](https://ads.tiktok.com/marketing_api/docs?id=1738235338194945). In this case, the CPM in your ad group is fixed. 
- If you set `rf_campaign_type` to `PULSE`, then the targeting locations cannot be updated in  [/adgroup/rf/update/](https://ads.tiktok.com/marketing_api/docs?id=1738235402874882). 
- If you set `rf_campaign_type` to `PULSE`, then `feed_type` cannot be `TOP_FEED` in  [/adgroup/rf/create/](https://ads.tiktok.com/marketing_api/docs?id=1738235338194945).|
| campaign_product_source {+Conditional}| string | Required when `objective_type` is set to `PRODUCT_SALES`. 
 Product source of the campaign. 
 Enum values: 
-  `CATALOG`: Catalog. 
- `STORE`: TikTok Shop, or TikTok Showcase.**Note**: 
...
-  For [Video Shopping Ads](https://ads.tiktok.com/marketing_api/docs?id=1741942291730434), the product source setting at the campaign level determines the product source available at the ad group level:  If this field is set to `CATALOG`, you need to set the `product_source` of ad groups under the campaign to `CATALOG`. 
...
Set this field to `true` only when you want to [create Automotive Ads for Inventory](https://business-api.tiktok.com/portal/docs?id=1827752507854850) or [Automotive Ads for Models](https://business-api.tiktok.com/portal/docs?id=1829635758164994).
...
|request_id|string|Request ID with which you can create campaigns with duplicate names. 
...
|budget_mode {+Conditonal}|string| Required when `objective_type` is **NOT** `RF_REACH`. 
...
|budget {+Conditional}|float|  Required when `budget_mode` is `BUDGET_MODE_DAY` or `BUDGET_MODE_DYNAMIC_DAILY_BUDGET` or `BUDGET_MODE_TOTAL`. 
...
| rta_bid_enabled | boolean | Whether to use RTA bid. 
 ...
 Supported values: 
-  `true`: enabled.
-  `false`: not enabled. Default value: `false`. 
...
- `DISABLE` : The campaign is disabled when created. Default value: `ENABLE`. 
If you want to update the status of the campaign after creation, use the [/campaign/status/update/](https://ads.tiktok.com/marketing_api/docs?id=1739320994354178) endpoint. |
| postback_window_mode | string | Valid only when `campaign_type` is `IOS14_CAMPAIGN` and `operation_status` is `DISABLE`. 

You can specify this field when you create a disabled campaign or when you disable an existing campaign by using [/campaign/status/update/](https://ads.tiktok.com/marketing_api/docs?id=1739320994354178). The recommended practice is to specify this field when disabling an existing campaign. 
...
Enum values: `TIKTOK_SHOP`: TikTok Shop. Drive sales on your TikTok Shop.
- `WEBSITE`: Website. Drive sales on your website.
- `APP`: App. Drive sales on your app (product catalog required).
...
- `EMPLOYMENT`: Ads for job offers, internship, professional certification programs or other related opportunities.
- `CREDIT`: Ads for credit card offers, auto loans, long-term financing or other related opportunities.|
#|budget_optimize_on|boolean| Returned only for campaigns with CBO enabled.
...
#|deep_bid_type |string| Bidding strategy for in-app events.
 ...
 For enum values and their descriptions, see [Enumeration - Deep-level Bidding Strategy](https://ads.tiktok.com/marketing_api/docs?id=1739381757818881).|
