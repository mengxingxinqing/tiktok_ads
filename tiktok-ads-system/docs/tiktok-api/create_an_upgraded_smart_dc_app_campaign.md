...
**Doc ID**: 1853377803841537
**Path**: Use Cases/Campaign creation/Create an Upgraded Smart+ Campaign/Create Upgraded Smart+ App Campaigns/Create an Upgraded Smart+ DC App Campaign
...
To learn about how to create an Upgraded Smart+ App Campaign with iOS 14 Dedicated Campaign disabled, see [Create an Upgraded Smart+ non-DC App Campaign](https://business-api.tiktok.com/portal/docs?id=1853377390815233). | 
            `campaign_type` | 
            `IOS14_CAMPAIGN` | 
         |
...
**Note**: If you enable this setting, you need to specify an App Profile Page at the ad level. | 
            `campaign_app_profile_page_state` | 
            `OFF` (or not specified) or `ON**Note**: If you set this field to `ON`, you need to specify an App Profile Page through `page_list` at the ad level. | 
         |
...
- ValueWe'll deliver your ads to users that are more likely to make a purchase in order to maximize your total purchase value.**Note**: When you select a campaign budget mode, this setting must be the same across ad groups within the same campaign. | 
            `optimization_goal` | 
            Any of the following values:
- `CLICK`
...
- Learn more about [Value-Based Optimization](https://business-api.tiktok.com/portal/docs?id=1739381743067137).**Note**: When`budget_optimize_on` is `true` at the campaign level, specify the same `optimization_goal`for ad groups within the same campaign. | 
         |
...
- Ensure the `activity_status` is `NO_RECENT_ACTIVITY` or `ACTIVE`.**Note**: When`budget_optimize_on` is `true` at the campaign level, the `optimization_event` and `custom_conversion_id`, if specified, must be the same for ad groups within the same campaign. | 
         |
...
- To use Ad revenue value, set `optimization_event` to `AD_REVENUE_VALUE`.**Note**: When`budget_optimize_on` is `true` at the campaign level, the `optimization_event`, if specified, must be the same for ad groups within the same campaign. | 
         |
...
- Day 0 ROAS or Highest Value**Note**: When you select a campaign budget mode, this setting, if specified, must be the same across ad groups within the same campaign. | 
            `vbo_window` | 
            
- When `optimization_goal` is `VALUE` , use any of the following values:`SEVEN_DAYS`
- `ZERO_DAY`For example, if you set `deep_bid_type` to `VO_MIN_ROAS`, specify `roas_bid`, and set `vbo_window` to `ZERO_DAY`, the system will aim to keep your average ROAS of the current day around or higher than the target ROAS value.**Note**: When`budget_optimize_on` is `true` at the campaign level, `vbo_window`, if specified, must be the same for ad groups within the same campaign: | 
         |
...
- If `optimization_goal` is `INSTALL`, `IN_APP_EVENT`,  or `VALUE`, set this field to `OCPM`.**Note**: When`budget_optimize_on` is `true` at the campaign level, specify the same `billing_event`for ad groups within the same campaign. | 
         |
...
- `UNSET`: To disable Goal-basedbudget increase.
...
- Set start and end time**Note**: You can only choose "Set start time and run ad group continuously" in any of the following scenarios: 
...
- To use standard call to action, specify `call_to_action_list`.**Note**: If you set `identity_type` within the `creative_info` object to `TT_USER`, `BC_AUTH_TT`, or `AUTH_CODE`, `call_to_action_id` is required. | 
         |
...
- Gift Code Sticker
- Display Card**Note**: 
- You can only use add-ons when `creative_list` contains video creatives, including videos and TikTok video posts.To create an interactive add-on (creative portfolio), use [/creative/portfolio/create/](https://business-api.tiktok.com/portal/docs?id=1739091950439426).
...
- When the "Use app profile page to optimize delivery" is enabled at the campaign level, Destination is App Profile and you need to use an App Profile Page.
- When the "Use app profile page to optimize delivery" is disabled at the campaign level: No configuration needed | 
            `landing_page_url_list`
`page_list` | 
            
- When `campaign_app_profile_page_state` at the campaign level is `ON`, you need to specify an App Profile Page via `page_list`.To obtain the page IDs of App Profile Pages within your ad account, use [/page/get/](https://business-api.tiktok.com/portal/docs?id=1820826387779586) and set `business_type` to `APP_PROFILE_PAGE`.
- To create an App Profile Page, you can use [Instant Page Editor SDK](https://business-api.tiktok.com/portal/docs?id=1740307202170881) and set `businessType` to `4`.
- Do not specify `landing_page_url_list`.
- When `campaign_app_profile_page_state` at the campaign level is `OFF` or not specified, you don't need to specify any configuration.Do not specify `landing_page_url_list` and `page_list`. | 
         |
