...
**Doc ID**: 1847302920924306
**Path**: Use Cases/Campaign creation/Create an Upgraded Smart+ Campaign/Create Upgraded Smart+ Web Campaigns/Create Upgraded Smart+ Streaming Ads

---

Upgraded Smart+ Streaming Ads are Upgraded Smart+ Web Campaigns that use catalog products from entertainment catalogs to deliver ads. 

For such campaigns, you need to set `catalog_enabled` to `true` and `catalog_type` to `TRAVEL_ENTERTAINMENT` at the campaign level and specify an entertainment catalog through `catalog_id` at the ad group level. By utilizing Upgraded Smart+ Streaming Ads, you can automatically generate ads in diverse formats such as catalog video, catalog carousel, and single video.

# Prerequisite
Creating Smart+ Streaming Ads requires setting up an entertainment catalog in Business Center first.

To set up the catalog:

1. Create an entertainment catalog using [/catalog/create/](https://business-api.tiktok.com/portal/docs?id=1740306481704961).
...
· Catalog | 
      Specify **an entertainment catalog with at least two approved products** | 
      `catalog_id`
...
- To obtain entertainment catalogs within a Business Center, use [/catalog/get/](https://business-api.tiktok.com/portal/docs?id=1740315452868610). For entertainment catalogs, the `catalog_type` should be `ENTERTAINMENT`.
- To learn about how to create an entertainment catalog, see [Prerequisite](#item-link-Prerequisite). | 
     |
...
- When `optimization_goal` is set to `CONVERT`, specify any of the following:A Standard Event through `optimization_event` .
...
To obtain the list of Custom Conversions associated with a Pixel, use [/custom_conversion/list/](https://business-api.tiktok.com/portal/docs?id=1842225174460673).
...
`product_set_id`
`product_ids` | 
      Set `product_specific_type` to `ALL` or `PRODUCT_SET`.
...
- Multiple ad format combinations are supported. For detailed configuration options and requirements, see [Customize the ad format combinations in Upgraded Smart+ Streaming Ads](#item-link-Customize the ad format combinations in Upgraded Smart+ Streaming Ads).
...
### Customize the ad format combinations in Upgraded Smart+ Streaming Ads
...
The possible ad format combinations for your Upgraded Smart+ Streaming Ad are illustrated in the following table.
...
| 
      ...
      Ad format combination for your Upgraded Smart+ Streaming Ad | 
     |
...
To ensure certain ad formats are available in Upgraded Smart+ Streaming Ads, follow the instructions in the following table.
