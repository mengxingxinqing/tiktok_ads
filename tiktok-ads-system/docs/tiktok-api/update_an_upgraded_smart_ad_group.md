...
**Doc ID**: 1843314894279682
**Path**: API Reference/Upgraded Smart+/Ad groups/Update an Upgraded Smart+ Ad Group
...
|bid_price|float|Valid when the following conditions are met:
...
`bid_price` needs to be lower than budget. See [Bidding-Bidding limits](https://business-api.tiktok.com/portal/docs?id=1745292444424193#item-link-Bidding%20limits) to learn more about the bid verification mechanism.
...
|
|conversion_bid_price|float|Valid when the following conditions are met:
...
`conversion_bid_price` needs to be lower than budget. See [Bidding-Bidding limits](https://business-api.tiktok.com/portal/docs?id=1745292444424193#item-link-Bidding%20limits) to learn more about the bid verification mechanism.
...
|
|roas_bid|float|Valid when `deep_bid_type` is `VO_MIN_ROAS`.
...
You can only set `schedule_type` to `SCHEDULE_START_END` in any of the following scenarios:
- At the campaign level, `budget_mode` is `BUDGET_MODE_TOTAL`
- At the ad group level, `budget_mode` is `BUDGET_MODE_TOTAL`|
|schedule_start_time|string|Schedule start time, in the format of `YYYY-MM-DD HH:MM:SS` (UTC+0).

The start time can be up to 12 hours earlier than the current time, but cannot be later than `2028-01-01 00:00:00`.|
|schedule_end_time|string|Valid when `schedule_type` is `SCHEDULE_START_END`.
...
**Note**: An all-0 value, an all-1 value and when this field is not specified, are considered full-time delivery.
|
...
- When you create other types of Smart+ Campaigns, `location_ids` is required.
...
#|zipcode_ids|string[]|
- When `catalog_type` is `TRAVEL_ENTERTAINMENT` at the campaign level, you need to set `location_ids` or `zipcode_ids` or both.
-  When you create other types of Smart+ Campaigns, `zipcode_ids` is not supported.
...
You can get an ID via the `app_package_id` field returned by [/pangle_block_list/get/](https://business-api.tiktok.com/portal/docs?id=1740039957181441). It only takes effect when Pangle placement is selected.|
