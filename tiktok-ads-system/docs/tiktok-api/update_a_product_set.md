...
**Doc ID**: 1740572974725122
**Path**: API Reference/Catalog Product Sets/Update a product set

---

Use this endpoint to update the filter conditions or name of a product set. You only need to include the fields that you want to update the values for.

## Comparing v1.2 and v1.3
 ```xtable
 |Changes{30%}|v1.2{35%}|v1.3{35%}|
|---|---|---|
|Endpoint path| /v1.2/catalog/set/update/| /v1.3/catalog/set/update/|
...
**Endpoint** https://business-api.tiktok.com/open_api/v1.3/catalog/set/update/
...
|product_set_id {Required}|string| The product set ID. |
|product_set_name{+Conditional}|string| You need to pass at least `product_set_name` or `conditions` or both.
 
The product set name. It must be less than 28 characters.|
|conditions{+Conditional}|object|You need to pass at least `product_set_name` or `conditions` or both.
...
#|and/or{+Conditional}|object[]|Required when `conditions` is passed. 
 
The logical operator that will be applied to the filter conditions. You must specify either `and` or `or`. `and` stands for the AND logical operator. `or` stands for the OR logical operator.|
##|field{+Conditional}|string|Required when `conditions` is passed. 
 
The field that the filter will be applied to. For enum values, see [Product set operators and fields – Filter fields](https://ads.tiktok.com/marketing_api/docs?id=1740673108747265).|
##|operation{+Conditional}|string|Required when `conditions` is passed. 
 
The filter operator. For enum values, see [Product set operators and fields – Operators](https://ads.tiktok.com/marketing_api/docs?id=1740673108747265).|
##|value{+Conditional}|string/string[]|Required when `conditions` is passed. 
...
https://business-api.tiktok.com/open_api/v1.3/catalog/set/update/
(/code)
```
