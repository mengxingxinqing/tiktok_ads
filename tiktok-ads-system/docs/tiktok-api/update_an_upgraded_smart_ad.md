...
**Doc ID**: 1843317411665921
**Path**: API Reference/Upgraded Smart+/Ads/Update an Upgraded Smart+ Ad
...
**Note**: The update mode of the `creative_list` parameter is full replacement. Even if you want to update one field within it, you need to specify all fields within `creative_list`. |
...
##|music_info {+Conditional}|object|Required when `catalog_creative_toggle` is `true`. 
...
|
##|identity_type|string|Valid when you create Spark Ads without using catalog creatives.
...
##|identity_id|string|Valid when you create Spark Ads without using catalog creatives.
...
|ad_text_list|object[]|List of ad texts.
...
- At the campaign level, `objective_type` is `APP_PROMOTION`, `WEB_CONVERSIONS`, or `objective_type` is `LEAD_GENERATION` and `catalog_enabled` is `false` or not specified.
...
**Note**: 
- You cannot remove a disclaimer that was previously added during ad creation, nor can you change its `disclaimer_type` between `TEXT_ONLY` and `TEXT_LINK`.
- You can only add a new disclaimer to an ad that does not already have one, or update the `text` (and `url`, if applicable) of an existing disclaimer.
|
...
#|identity_type|string|Valid in any of the following scenarios:
...
For details about identities, see [Identities](https://business-api.tiktok.com/portal/docs?id=1738958351620097).|
#|identity_id|string|Valid in any of the following scenarios:
- You are creating non-Spark Ads on non-TikTok placements. Non-Spark Ads on the TikTok placement are no longer supported.
- You are creating Spark Ads using catalog creatives from an E-commerce catalog. Learn more about how you can create such ads in [Create Upgraded Smart+ E-commerce Catalog Ads](https://business-api.tiktok.com/portal/docs?id=1847302895272962).
Identity ID.|
...
- Mandatory "Show through ads only" mode for an ad account is currently an allowlist-only feature. If you would like to access it, please contact your TikTok representative. 
 |
#|product_specific_type|string|Valid when `catalog_enabled` is `true` at the campaign level.
...
**Note**:  If `catalog_media_settings` is specified during ad creation, you cannot update `catalog_creative_toggle` to `false` or update `catalog_media_settings` to an empty list (`[]`).
|
...
**Note**: 
- Once an ad group is created, the `TRANSLATE_AND_DUB` option within `creative_auto_enhancement_strategy_list` cannot be updated. You cannot add it if it was initially excluded, nor remove it if it was included.
- The automatic enhancements for images (`IMAGE_QUALITY` and `IMAGE_RESIZE`) will only take effect if your ad contains image creatives.
|
...
#|end_card_cta|string|Valid when the following conditions are met:
...
- `__PLACEMENT__`: This will be replaced by your placement.
Length limit when you specify a custom value: 600 characters.|
#| product_info{+Conditional} | object | Product information.
...
**Note**: The update mode of the `product_info` parameter is full replacement. Even if you want to update one field within it, you need to specify all fields within `product_info`. When `product_info` is updated, the ad will be reviewed again. |
...
**Note**: The update mode of the `promo_info_list` parameter is full replacement. Even if you want to update one field within it, you need to specify all fields within `promo_info_list`. When `promo_info_list` is updated, the ad will be reviewed again. |
...
##|tracking_app_id|string|Valid when `objective_type` is `WEB_CONVERSIONS`.
