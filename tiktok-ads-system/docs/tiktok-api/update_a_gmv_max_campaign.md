...
**Doc ID**: 1822001009002497
**Path**: API Reference/GMV Max/Update a GMV Max Campaign
...
| roas_bid | number | Valid only when `deep_bid_type` of the campaign is `VO_MIN_ROAS`.
...
| item_group_ids | string[] | Valid when `shopping_ads_type` is `PRODUCT` and `product_specific_type` is `CUSTOMIZED_PRODUCTS`. 
...
|affiliate_posts_enabled |boolean|Valid only when the following conditions are both met:
...
Supported values: `true`, `false`.|
| item_list  | object[] | Valid when `shopping_ads_type` is `PRODUCT` and `product_video_specific_type` is `CUSTOM_SELECTION`. 
...
#| spu_id_list {+Conditional} | string[] | Required when `item_list` is passed. 
 ...
 Product SPU IDs associated with the TikTok post. |
...
**Note**: 
- If you want to specify more than 200 customized posts for a Product GMV Max Campaign, you can update the `custom_anchor_video_list` of the campaign incrementally, with updates limited to 200 additional posts per request. For example, if you created the campaign with 200 customized posts and want to increase the post count to 400, specify 400 customized posts including the original 200 and the additional 200 ones through this field. 
- If you want to remove certain customized posts from the Product GMV Max Campaign, make sure to remove the posts from both `custom_anchor_video_list` and `item_list` simultaneously. |
